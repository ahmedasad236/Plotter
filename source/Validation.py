...
#-------------------------------------------------------------------------------------    
    def validateMaxMinValues(self, minValue, maxValue):
        if minValue >= maxValue:
            raise ValueError("Maximum must be greater than Minimum")
#-------------------------------------------------------------------------------------
    def validateDivisionByZero(self, expression, minValue):
        if expression.find('/X') != -1 or expression.find('/x') != -1 and minValue <= 0:
            return False
        return True
